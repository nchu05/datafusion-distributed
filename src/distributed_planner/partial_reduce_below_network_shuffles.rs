...
/// Inserts [`AggregateMode::PartialReduce`] below the hash [`RepartitionExec`] that feeds
/// the network boundary, with its own inner hash [`RepartitionExec`] to merge partial-aggregate
/// partitions by key before reducing. The original [`RepartitionExec`] stays in place above.
pub(crate) fn partial_reduce_below_network_shuffles(
    ...
    let transformed = plan.transform_up(|plan| {
        ...
        // Inner RepartitionExec merges AggPartial's output partitions by hash key,
        // so PartialReduce can combine multiple partial rows per key into one.
        let inner_n = plan.output_partitioning().partition_count();
        let inner_partitioning = match repartition.partitioning() {
            Partitioning::Hash(exprs, _) => Partitioning::Hash(exprs.clone(), inner_n),
            _ => return Ok(Transformed::no(plan)),
        };
        let inner_repartition: Arc<dyn ExecutionPlan> = Arc::new(
            RepartitionExec::try_new(Arc::clone(&grandchild), inner_partitioning)?,
        );
        ...
        // Rebuild the original RepartitionExec with PartialReduce as its new child,
